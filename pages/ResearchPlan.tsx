...
import { 
    ...
    BarChart3, FileText, Send, X, Edit, Trash2, Save, Archive, Settings, RotateCcw
} from 'lucide-react';
...
export const ResearchPlanPage: React.FC<ResearchPlanPageProps> = ({ user }) => {
    ...
    const [isPlanModalOpen, setIsPlanModalOpen] = useState(false); // مودال الخطة
    ...
    const fetchData = async () => {
        ...
        // 2. جلب الأرشيف (كل الخطط واستبعاد النشطة)
    ...
    const handleOpenPlanModal = (plan?: ResearchPlan) => {
        if (plan) {
            ...
            // إنشاء خطة جديدة (سيؤرشف القديمة)
    ...
    const handleSavePlan = async (e: React.FormEvent) => {
        ...
        const planData = {
            ...
            axes: editingPlanId && activePlan ? activePlan.axes : [] // الحفاظ على المحاور عند التعديل
        };

        try {
            if (editingPlanId) {
                await updateResearchPlan(editingPlanId, planData);
                await logActivity('تعديل الخطة البحثية', user.name, `تعديل بيانات الخطة: ${planForm.title}`);
            } else {
                await addResearchPlan(planData); // هذا سيؤرشف الخطة القديمة تلقائياً في dbService
                await logActivity('إنشاء خطة بحثية', user.name, `إطلاق خطة جديدة: ${planForm.title}`);
            }
    ...
    const handleDeletePlan = async () => {
        ...
        if (!window.confirm('تحذير هام: هل أنت متأكد من حذف الخطة البحثية الحالية بالكامل؟ يفضل أرشفتها بإنشاء خطة جديدة بدلاً من الحذف.')) return;
        await deleteResearchPlan(activePlan.id);
        await logActivity('حذف خطة بحثية', user.name, `حذف الخطة: ${activePlan.title}`);
        fetchData();
    };
    ...
    const handleProposalSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const proposalData = {
                ...
                studentName: proposalForm.hasStudent ? proposalForm.studentName : "", // Fix: Send empty string instead of undefined
            ...
            console.error(error);
            alert('حدث خطأ أثناء الإرسال. تأكد من ملء جميع البيانات.');
        }
    };
            ...
            <div className="relative z-10">
                {activePlan ? (
                    ...
                    <div className="text-center py-8">
                        ...
                        {isAdmin && (
                            <button onClick={() => handleOpenPlanModal()} className="bg-white text-green-800 px-6 py-3 rounded-xl font-bold hover:bg-green-50 transition shadow-lg flex items-center gap-2 mx-auto">
                                <Plus className="w-5 h-5"/> إنشاء الخطة الخمسية للقسم
                            </button>
                        )}
                    </div>
                )}
    ...
    const renderBrowseTab = () => (
        <div className="space-y-6 animate-in fade-in">
            {!activePlan ? <div className="text-center text-gray-500">يرجى إنشاء خطة أولاً</div> : (
                <>
                    {/* Search & Filter */}
                    ...
                    <div key={prop.id} className="bg-white border p-4 rounded-lg shadow-sm flex flex-col md:flex-row justify-between gap-4">
                        ...
                        <div className="flex flex-col gap-2 min-w-[120px] justify-center">
                            {isAdmin && prop.status === 'PENDING' ? (
                                ...
                                // زر التعديل يظهر لصاحب المقترح في حالة "قيد المراجعة" أو "مطلوب تعديل"
                                (prop.proposedById === user.id && (prop.status === 'PENDING' || prop.status === 'MODIFICATION_REQUESTED')) && (
                                    <div className="flex gap-2">
                                        <button onClick={() => handleEditProposal(prop)} className="text-blue-500 hover:bg-blue-50 p-2 rounded flex items-center gap-1 text-sm font-bold border border-blue-200">
                                            <Edit className="w-4 h-4"/> تعديل وإعادة إرسال
                                        </button>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex border-b border-gray-200 bg-white rounded-t-xl overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('ARCHIVE')} className={`px-6 py-4 font-bold text-sm flex items-center gap-2 transition-colors ${activeTab === 'ARCHIVE' ? 'text-green-700 border-b-2 border-green-700 bg-green-50' : 'text-gray-500 hover:text-green-600'}`}>
            ...
            {isPlanModalOpen && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-2xl p-6 shadow-2xl animate-in fade-in">
                         <div className="flex justify-between items-center mb-6 border-b pb-4">
                            <h3 className="text-xl font-bold text-gray-800">
                                {editingPlanId ? 'تعديل الخطة الحالية' : 'إنشاء خطة بحثية جديدة'}
                            </h3>
                            <button onClick={() => setIsPlanModalOpen(false)}><X className="w-6 h-6 text-gray-400 hover:text-red-500"/></button>
                        </div>
                        <form onSubmit={handleSavePlan} className="space-y-4">
                            {editingPlanId === null && (
                                <div className="bg-yellow-50 p-3 rounded border border-yellow-200 text-sm text-yellow-800 mb-4">
                                    ⚠️ تنبيه: إنشاء خطة جديدة سيؤدي تلقائياً إلى أرشفة الخطة الحالية.
                                </div>
                            )}
                            <div>
                                <label className="block text-sm font-bold mb-1">عنوان الخطة</label>
                                <input type="text" required className="w-full border p-2 rounded" value={planForm.title} onChange={e => setPlanForm({...planForm, title: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-sm font-bold mb-1">الرؤية البحثية</label>
                                <textarea required className="w-full border p-2 rounded" rows={3} value={planForm.vision} onChange={e => setPlanForm({...planForm, vision: e.target.value})} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold mb-1">من تاريخ</label>
                                    <input type="date" required className="w-full border p-2 rounded" value={planForm.startDate} onChange={e => setPlanForm({...planForm, startDate: e.target.value})} />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold mb-1">إلى تاريخ</label>
                                    <input type="date" required className="w-full border p-2 rounded" value={planForm.endDate} onChange={e => setPlanForm({...planForm, endDate: e.target.value})} />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-bold mb-1">الأهداف الاستراتيجية (افصل بينها بفاصلة)</label>
                                <input type="text" className="w-full border p-2 rounded" value={planForm.strategicGoals} onChange={e => setPlanForm({...planForm, strategicGoals: e.target.value})} />
                            </div>
                            <div className="flex justify-end gap-3 pt-4">
                                <button type="button" onClick={() => setIsPlanModalOpen(false)} className="px-4 py-2 text-gray-600">إلغاء</button>
                                <button type="submit" className="bg-green-600 text-white px-6 py-2 rounded font-bold hover:bg-green-700">حفظ الخطة</button>
                            </div>
                        </form>
                    </div>
                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    >
                                        ...
                                        {activePlan && activePlan.axes.map(ax => <option key={ax.id} value={ax.id}>{ax.title}</option>)}
                                ...
                                {/* New Axis Name Input */}
                                {proposalForm.axisId === 'NEW' && (
                                    <div className="col-span-2 animate-in fade-in">
                                        <label className="block text-sm font-bold text-purple-700 mb-1">اسم المحور الجديد المقترح</label>
                                        <input 
                                            type="text" required 
                                            className="w-full border p-3 rounded-lg border-purple-300 focus:ring-2 focus:ring-purple-500 outline-none bg-purple-50"
                                            placeholder="اكتب اسم المحور الجديد..."
                                            value={proposalForm.newAxisName}
                                            onChange={e => setProposalForm({...proposalForm, newAxisName: e.target.value})}
                                        />
                                    </div>
                                )}
                            </div>
