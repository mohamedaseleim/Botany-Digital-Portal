...
import { 
    ...
    UploadCloud, Loader2, X, User, Edit, Trash2, Save, ShieldCheck
} from 'lucide-react';
import { User as UserType, LeaveRequest, LeaveType, UserRole } from '../types';
import { 
    addLeaveRequest, getMyLeaves, getSubstituteRequests, respondToSubstituteRequest, 
    getStaff, getEmployees, getAllLeaves, uploadFileToDrive, logActivity 
...
export const LeaveManagement: React.FC<LeaveManagementProps> = ({ user }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const substituteName = potentialSubstitutes.find(s => s.id === formData.substituteId)?.name;
        ...
        return (
            <div className="space-y-6 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {/* Admin Card */}
                    {isAdmin && (
                        <div className="md:col-span-3 bg-green-50 p-6 rounded-xl border border-green-200 shadow-sm cursor-pointer hover:bg-green-100 transition-colors" onClick={() => setActiveTab('ALL_REQUESTS')}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {(formData.type === 'CASUAL' || formData.type === 'ANNUAL' || formData.type === 'HAJJ') && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in">
                        <div><label className="block text-sm font-bold text-gray-700 mb-1">العنوان أثناء الإجازة</label><input type="text" required className="w-full p-2 border rounded-lg" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} /></div>
                        <div><label className="block text-sm font-bold text-gray-700 mb-1">رقم الطوارئ</label><input type="tel" required className="w-full p-2 border rounded-lg" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} /></div>
                    </div>
                )}

                {formData.type === 'SICK' && (
                    <div className="space-y-4 animate-in fade-in">
                        <div><label className="block text-sm font-bold text-gray-700 mb-1">مكان العلاج</label><input type="text" placeholder="مستشفى / عيادة / منزل" required className="w-full p-2 border rounded-lg" value={formData.hospital} onChange={e => setFormData({...formData, hospital: e.target.value})} /></div>
                        <div className="border-2 border-dashed border-gray-300 p-4 rounded-lg text-center"><input type="file" className="hidden" id="med-report" onChange={e => setFiles(Array.from(e.target.files || []))} /><label htmlFor="med-report" className="cursor-pointer flex flex-col items-center gap-2"><Stethoscope className="w-8 h-8 text-red-500"/><span className="text-sm font-bold text-gray-600">رفع التقرير الطبي</span>{files.length > 0 && <span className="text-xs text-green-600">{files[0].name}</span>}</label></div>
                    </div>
                )}

                {formData.type === 'SCIENTIFIC' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in">
                        <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1">اسم المؤتمر / الجهة</label><input type="text" required className="w-full p-2 border rounded-lg" value={formData.conferenceName} onChange={e => setFormData({...formData, conferenceName: e.target.value})} /></div>
                        <div><label className="block text-sm font-bold text-gray-700 mb-1">المكان / الدولة</label><input type="text" required className="w-full p-2 border rounded-lg" value={formData.conferenceLocation} onChange={e => setFormData({...formData, conferenceLocation: e.target.value})} /></div>
                        <div className="border-2 border-dashed border-gray-300 p-2 rounded-lg text-center flex items-center justify-center"><input type="file" className="hidden" id="conf-invite" onChange={e => setFiles(Array.from(e.target.files || []))} /><label htmlFor="conf-invite" className="cursor-pointer text-sm text-blue-600 hover:underline">{files.length > 0 ? files[0].name : 'رفع دعوة المؤتمر'}</label></div>
                    </div>
                )}
                ...
                {formData.type === 'SPOUSE' && (
                    <div className="space-y-4 animate-in fade-in bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="border-2 border-dashed border-gray-300 p-4 rounded-lg text-center"><input type="file" multiple className="hidden" id="spouse-docs" onChange={e => setFiles(Array.from(e.target.files || []))} /><label htmlFor="spouse-docs" className="cursor-pointer flex flex-col items-center gap-1"><UploadCloud className="w-6 h-6 text-gray-400"/><span className="text-sm text-gray-600">رفع المستندات</span>{files.length > 0 && <span className="text-xs text-green-600">{files.length} ملفات</span>}</label></div>
                    </div>
                )}
    ...
    const renderAllRequests = () => (
        <div className="space-y-4 animate-in fade-in">
            <h3 className="font-bold text-gray-700 mb-4 flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-green-600"/> جميع طلبات الإجازات (للمدير)</h3>
            {allLeaves.length === 0 && <div className="text-center p-12 bg-gray-50 rounded-xl text-gray-400">لا توجد طلبات</div>}
            
            <div className="grid gap-4">
                {allLeaves.map(req => (
                    <div key={req.id} className="bg-white border border-gray-200 shadow-sm rounded-xl p-4 flex flex-col md:flex-row justify-between gap-4 hover:shadow-md transition-shadow">
                        <div>
                        ...
                        {req.status === 'PENDING_HEAD' && (
                            <div className="flex gap-2 items-center self-start md:self-center">
                                <button onClick={() => handleAdminAction(req.id, 'APPROVED')} className="bg-green-600 text-white px-4 py-2 rounded font-bold hover:bg-green-700 shadow-sm">اعتماد</button>
                                <button onClick={() => handleAdminAction(req.id, 'REJECTED')} className="bg-red-600 text-white px-4 py-2 rounded font-bold hover:bg-red-700 shadow-sm">رفض</button>
                            </div>
                        )}
                    </div>
                ))}
            ...
            <div className="grid gap-4">
                {subRequests.map(req => (
                    <div key={req.id} className="bg-white border-l-4 border-purple-500 shadow-sm rounded-r-xl p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            <div className="flex items-center gap-2 mb-1"><span className="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded-full font-bold">طلب بديل</span><h4 className="font-bold text-gray-800 text-lg">{req.userName}</h4></div>
                            <p className="text-gray-600 text-sm">يرغب في إجازة <span className="font-bold">{getLeaveLabel(req.type)}</span></p>
                            <div className="flex gap-4 mt-2 text-sm text-gray-500 font-mono"><span>من: {req.startDate}</span><span>إلى: {req.endDate}</span><span>({req.daysCount} أيام)</span></div>
                        </div>
                        <button onClick={() => { setSelectedSubRequest(req); setSubAction(null); }} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg font-bold shadow-md">عرض التفاصيل واتخاذ قرار</button>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="space-y-4 animate-in fade-in">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <table className="w-full text-sm text-right">
                    <thead className="bg-gray-50 text-gray-600"><tr><th className="p-4">نوع الإجازة</th><th className="p-4">من</th><th className="p-4">إلى</th><th className="p-4">البديل</th><th className="p-4">الحالة</th><th className="p-4">إجراءات</th></tr></thead>
                    <tbody className="divide-y">
                        {myLeaves.map(leave => (
                            <tr key={leave.id} className="hover:bg-gray-50">
                                ...
                                <td className="p-4"><span className={`px-2 py-1 rounded-full text-xs font-bold ${leave.status === 'APPROVED' ? 'bg-green-100 text-green-700' : leave.status === 'REJECTED' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{leave.status === 'APPROVED' ? 'مقبولة' : leave.status === 'PENDING_SUBSTITUTE' ? 'انتظار البديل' : leave.status === 'PENDING_HEAD' ? 'انتظار المدير' : 'مرفوضة'}</span></td>
                                <td className="p-4 flex gap-2">
                                    {(leave.status === 'PENDING_SUBSTITUTE' || leave.status === 'PENDING_HEAD') && (<><button onClick={() => handleEditRequest(leave)} className="text-blue-600 hover:bg-blue-50 p-1.5 rounded"><Edit className="w-4 h-4"/></button><button onClick={() => handleDeleteRequest(leave.id, leave.type)} className="text-red-600 hover:bg-red-50 p-1.5 rounded"><Trash2 className="w-4 h-4"/></button></>)}
                                    {leave.status === 'APPROVED' && <button onClick={() => window.print()} className="text-green-600 hover:underline flex items-center gap-1"><FileText className="w-3 h-3"/> طباعة</button>}
                                </td>
                            </tr>
                        ))}
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex border-b border-gray-200 bg-white rounded-t-xl overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('SUBSTITUTE')} className={`px-6 py-4 font-bold text-sm transition-colors flex items-center gap-2 ${activeTab === 'SUBSTITUTE' ? 'text-purple-700 border-b-2 border-purple-700 bg-purple-50' : 'text-gray-500 hover:text-purple-600'}`}>طلبات البديل {subRequests.length > 0 && <span className="bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full">{subRequests.length}</span>}</button>
                ...
                {isAdmin && <button onClick={() => setActiveTab('ALL_REQUESTS')} className={`px-6 py-4 font-bold text-sm transition-colors flex items-center gap-1 ${activeTab === 'ALL_REQUESTS' ? 'text-green-700 border-b-2 border-green-700' : 'text-gray-500 hover:text-green-600'}`}><ShieldCheck className="w-4 h-4"/> كل الطلبات</button>}
            </div>
